...
1) 텍스트마이닝: 인스타그램 해시태그 크롤링을 통한 분석
2) 웹 로그 분석: 구글 애널리틱스
- 2020.8 ~ 현재 대학생 연합 빅데이터 동아리 BITAmin: 
3) 1학기 발표: 워드클라우드, 시계열데이터, AutoML, 캐글 성인 인구조사 소득 예측 대회
- 2020.8 ~ 2021.1 숭실대학교 창업동아리 '바로다스리기'팀 명상&헬스케어 어플리케이션 '까미' 데이터분석/기획운영:
4) 스마트폰 심박수 측정을 이용한 명상 컨텐츠 기획
5) 데이터가 흐르는 조직 만들기(그로스해킹) 발표
6) Agile적 방식으로 ESG를 고려한 비즈니스 모델 논의
7) Amplitude, Google Analytics, Google Bigquery, Tableau, Redash 등 툴 도입 검토
- 2021.1 공공데이터 청년인턴 전문교육(중고급) 이수
- 2020.12 공공데이터 청년인턴십 수료 (발행주체 : 한국정보화진흥원 원장)
- 2020.9 ~ 2020. 12 효성ITX 인턴: 
8) 한국벤처투자 모태펀드와 관련하여 자펀드 및 운용사, 투자실적 등 데이터 정비 업무  
9) 공공데이터의 가공과 효율적인 데이터 추출을 위한 기술조사 및 데이터 샘플 제작 업무
10) 파이썬을 활용한 공공데이터 생산 효율성 검토
11) 모태자펀드 규약(PDF파일 등) 내용 DB화 작업 보조
12) AI 시스템 구축 업체 '애니파이브' 업무회의 참석 - 머신러닝 기반 추천시스템 논의
...
13) 공공데이터를 활용한 '상가 부동산 입지 선정' 프로젝트
14) 공유오피스 위워크/패스트파이브 비즈니스 모델 조사
